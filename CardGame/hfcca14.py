...
"""
source_analyzer (verion 1.4) is a simple code complexity counter without caring about the C/C++ header files.
...
class objc_file_parser(c_file_parser):
    ...
    def _OBJC_DEC_BEGIN(self, token):
        if token == ':':
            self.add_to_function_name(token)
...
def main(argv):
    ...
    if options.no_preprocessor_count:
        remove_sharp_from_class(c_file_parser)
        remove_sharp_from_class(sdl_file_parser)
...
class Test_objc_hfcca(unittest.TestCase):
    ...
    def test_one_objc_function_with_three_param(self):
        result = create_objc_hfcca("- (id)initWithRequest:(NSURLRequest *)request delegate:(id <NSURLConnectionDelegate>)delegate startImmediately:(BOOL)startImmediately{}")
        self.assertTrue("initWithRequest: delegate: startImmediately:" in result)
